...
// Logging target: the EXTERNAL ZD25WQ80C SPI flash only — internal STM32
// flash is never erased or written for logging. Logs start at external
// address 0 and run until the whole 1MB chip is full (uncapped).
uint32_t log_write_addr = 0;             // next external-flash address to program
uint32_t log_erased_sector = 0xFFFFFFFFU; // sentinel: no sector erased yet
uint8_t  log_page_buf[ZD25WQ80C_PAGE_SIZE];
uint16_t log_page_index = 0;

// Sector-erase (4KB) the sector containing `addr`, but only once per
// session — the log write pointer only ever moves forward.
static void ensureLogSectorErased(uint32_t addr) {
...
/* USART1_Transmit() is provided by VCP.c (declared in VCP.h) */

void sendToSimulink(){
    USART1_Transmit((uint8_t *) "J_A"           ,3);

    // Switches
    USART1_Transmit((uint8_t*)  &SW1     ,4);
    USART1_Transmit((uint8_t *) &SW2     ,4);

  // IMU
    USART1_Transmit((uint8_t*) &IMU_Accel[0]     ,4);
    USART1_Transmit((uint8_t*) &IMU_Accel[1]     ,4);
    USART1_Transmit((uint8_t*) &IMU_Accel[2]     ,4);
    USART1_Transmit((uint8_t*) &IMU_Gyro[0]     ,4);
    USART1_Transmit((uint8_t*) &IMU_Gyro[1]     ,4);
    USART1_Transmit((uint8_t*) &IMU_Gyro[2]     ,4);
    USART1_Transmit((uint8_t*) &IMU_Temp         ,4);


  // TOF
    USART1_Transmit((uint8_t*) &(TOF_sb_left_result.Distance)  , 4);
    USART1_Transmit((uint8_t*) &(TOF_sb_left_result.Ambient )  , 4);
    USART1_Transmit((uint8_t*) &(TOF_sb_left_result.Signal  )  , 4);
    USART1_Transmit((uint8_t*) &(TOF_sb_left_result.Status  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_left_result.Distance)  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_left_result.Ambient )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_left_result.Signal  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_left_result.Status  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_result.Distance)  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_result.Ambient )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_result.Signal  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_result.Status  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_right_result.Distance)  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_right_result.Ambient )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_right_result.Signal  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_front_right_result.Status  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_right_result.Distance)  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_right_result.Ambient )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_right_result.Signal  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_sb_right_result.Status  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_mb_back_result.Distance)  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_mb_back_result.Ambient )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_mb_back_result.Signal  )  , 4);
    USART1_Transmit((uint8_t*)  &(TOF_mb_back_result.Status  )  , 4);

    // VBAT, MOT_RS , DOWN_RS , DOWN_LS , MOT_LS
    USART1_Transmit((uint8_t*)  &(ADCs[0]  )  , 2);
    USART1_Transmit((uint8_t*)  &(ADCs[0]  )  , 2);

    USART1_Transmit((uint8_t*)  &(ADCs[1]  )  , 2);
    USART1_Transmit((uint8_t*)  &(ADCs[1]  )  , 2);

    USART1_Transmit((uint8_t*)  &(ADCs[2]  )  , 2);
    USART1_Transmit((uint8_t*)  &(ADCs[2]  )  , 2);

    USART1_Transmit((uint8_t*)  &(ADCs[3]  )  , 2);
    USART1_Transmit((uint8_t*)  &(ADCs[3]  )  , 2);

    USART1_Transmit((uint8_t*)  &(ADCs[4]  )  , 2);
    USART1_Transmit((uint8_t*)  &(ADCs[4]  )  , 2);

    // INA219
    USART1_Transmit((uint8_t *) &Vbattery     ,2);
    USART1_Transmit((uint8_t *) &Vshunt       ,2);
    USART1_Transmit((uint8_t *) &Current      ,2);
    USART1_Transmit((uint8_t *) &Power        ,2);
    USART1_Transmit((uint8_t *) &batteryLife  ,1);
    USART1_Transmit((uint8_t *) &batteryLife  ,1);
    USART1_Transmit((uint8_t *) &batteryLife  ,1);
    USART1_Transmit((uint8_t *) &batteryLife  ,1);
    ...
    USART1_Transmit((uint8_t *) &counter             ,4);

    // Motor encoder rates (int16, RPM, signed)
    USART1_Transmit((uint8_t *) &MOTOR_L.encoderRate ,2);
    USART1_Transmit((uint8_t *) &MOTOR_R.encoderRate ,2);
    // USART1_Transmit((uint8_t *) "A_J"      ,3);
}

void recievedFromSimulink(){
    // Data has been received via circular DMA into bigBuffer
    ...
    // DMA2 Ch7 is circular - reception restarts automatically
}
...
void initLogs() {
    ...
    // Logging always starts fresh at external flash address 0 — no old-log
    // scan needed, since this is a dedicated logging chip, not shared with
    // application code the way internal flash was.
    log_write_addr = 0;
...
void refreshLoggedData() {
    ...
    if (first_buffer) {
        ...
        // Logging always starts at external flash address 0 (matching
        // initLogs()); erase-as-you-go handles the rest as bytes are written.
        log_write_addr = 0;
        log_erased_sector = 0xFFFFFFFFU;
        ensureLogSectorErased(log_write_addr);

        // Write metadata header at the start of the very first page
        ...
        memcpy(log_page_buf, &header, sizeof(MicroMouseLogHeader_t));
        log_page_index = sizeof(MicroMouseLogHeader_t);
    ...
    // Write log to buffer, handling the external flash's 256-byte page
    // boundary with partial writes and flushing.
    uint8_t *log_bytes = (uint8_t*)&log;
    uint16_t log_offset = 0;

    while (log_offset < sizeof(MicroMouseLog_t)) {
        // Calculate how much space is left in the current page buffer
        uint16_t page_space = ZD25WQ80C_PAGE_SIZE - log_page_index;

        // Write as much as we can (either remaining log bytes or remaining page space)
        uint16_t to_write = (sizeof(MicroMouseLog_t) - log_offset < page_space) ?
                            (sizeof(MicroMouseLog_t) - log_offset) : page_space;
        ...
        // If the page buffer is full (256 bytes), program it and advance
        if (log_page_index >= ZD25WQ80C_PAGE_SIZE) {
            ...
            uint32_t flash_used = log_write_addr;
            uint32_t flash_available = ZD25WQ80C_FLASH_SIZE;
            ...
            // Check if the entire external flash chip is now full
...
void main(void)
{
  ...
  /* Start USART1 RX via DMA2 Ch7 (circular) */
  VCP_Init();
  ...
  while (1)
  {
    ...
    // Parse incoming command packet if header is present in the circular DMA buffer
...
void MX_TIM3_Init(void)
{
  ...
  htim3.Instance = TIM3;
  htim3.Init.Prescaler = 4-1;          // 80 MHz / 4 = 20 MHz timer clock
  htim3.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim3.Init.Period = 1000-1;          // 20 MHz / 1000 = 20 kHz PWM
  ...
  // 4-channel PWM: CH1=Right-Fwd, CH2=Right-Bwd, CH3=Left-Fwd, CH4=Left-Bwd
  sConfigOC.OCMode = TIM_OCMODE_PWM1;
  sConfigOC.Pulse = 0;
  sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
  sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
...
void MX_TIM4_Init(void)
{
  ...
  htim4.Instance = TIM4;
  htim4.Init.Prescaler = 80-1;          // 80 MHz / 80 = 1 MHz (1 µs per tick)
  htim4.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim4.Init.Period = 0xFFFF;            // 16-bit wrap at 65535 µs (~65 ms max period)
  ...
  // Input capture: rising edge, direct TI, div1, digital noise filter
  sConfigIC.ICPolarity  = TIM_ICPOLARITY_RISING;
  sConfigIC.ICSelection = TIM_ICSELECTION_DIRECTTI;
  sConfigIC.ICPrescaler = TIM_ICPSC_DIV1;
  sConfigIC.ICFilter    = 0x0F;  // 8 samples at fDTS/16: ~500 kHz cutoff
...
void MX_USART1_UART_Init(void)
{
  ...
  */
  ...
  /* USART1_RX Init - DMA2 Ch7 circular */
  ...
  /* USART1_TX Init - DMA2 Ch6 normal */
...
/**
  * @brief SPI2 Initialization Function (ZD25WQ80C external flash)
  * @retval None
  */
...
/* USER CODE BEGIN 4 */
/* HAL_UART_RxCpltCallback is in VCP.c */
/* USER CODE END 4 */
